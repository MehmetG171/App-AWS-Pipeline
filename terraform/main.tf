...
resource "aws_s3_bucket" "terraform_state_bucket" {
  bucket = "mng-terraform-state-bucket-${terraform.workspace}"
  acl    = "private"
...
terraform {
  backend "s3" {
    bucket = "mng-terraform-state-bucket-dev"  ## actual bucket name
...
resource "aws_ecr_repository" "app_repo" {
  name                 = "mngapprepo${terraform.workspace}"
...
resource "aws_instance" "ubuntu_instance" {
  ...
  tags = {
    Name = "mng-ec2-${terraform.workspace}"
  }
